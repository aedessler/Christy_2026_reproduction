...
The original Fortran performs IDW regridding of station data to a 0.5° half-degree grid
before computing the CONUS average. The required grid mask (`usreg_half.txt`) was not
included in the data archive, so it is regenerated here using `make_conus_mask.py` from
the Census Bureau CONUS boundary (see Scripts below). The `usreg_half.txt` file in this
repository is our own reconstruction — it is not from Christy or the UAH archive.
...
| `make_conus_mask.py` | One-time script that generates `usreg_half.txt` (CONUS grid mask) |

### Grid mask

| File | Description |
|------|-------------|
| `usreg_half.txt` | 116×50 half-degree CONUS mask (50 rows N→S, 116 cols W→E; 1=inside CONUS, 0=outside). **Not from Christy/UAH** — generated by `make_conus_mask.py` using the Census Bureau national boundary shapefile. |
...
pandas
matplotlib
geopandas   # only needed to run make_conus_mask.py
shapely     # only needed to run make_conus_mask.py
```
...
> **Note:** CONUS grid averages require `usreg_half.txt`. If it is present (generated by
> `make_conus_mask.py`), IDW-weighted CONUS averages are computed and printed in the
> `US` column of the state table. If absent, that column shows `-99`.
...
> averages. This script uses the same approach via `usreg_half.txt` (generated by
> `make_conus_mask.py`).
...
Scripts reproduce these within ~5%.
